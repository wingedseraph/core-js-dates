...
function dateToTimestamp(date) {
  return new Date(date).getTime() - new Date(0).getTime();
}
...
function getNextFriday(date) {
  ...
  const daysToFriday = (5 - startDate.getDay() + 7) % 7 || 7;

  const nextFriday = new Date(startDate);
  nextFriday.setUTCDate(startDate.getDate() + daysToFriday);
...
function isDateInPeriod(date, period) {
  const checkedDate = new Date(`${date}T00:00:00Z`);
